...
class _AccuracyScreenState extends State<AccuracyScreen> {
  ...
  Future<void> _refreshSessionsForSelectedIteration({required bool showChartLoader}) async {
    final user = _user;
    ...
    try {
      ...
      final sessionsSnap = await firestore.collection('iterations').doc(user.uid).collection('iterations').doc(iterationId).collection('sessions').orderBy('date', descending: false).get();

      final sessions = await _loadSessionsWithShots(sessionsSnap.docs);
  ...
  Widget _buildBody(BuildContext context, User user) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snap) {
        ...
        if (_isInitialLoading && _sessions.isEmpty) {
          return Center(
            child: CircularProgressIndicator(
              strokeWidth: 3.2,
              valueColor: AlwaysStoppedAnimation<Color>(Theme.of(context).primaryColor),
  ...
  Widget _buildRadialAccuracyChart(BuildContext context, String? iterationId, List<QueryDocumentSnapshot> iterationDocs) {
    final shotTypes = ['wrist', 'backhand', 'slap', 'snap'];

    if (iterationId == null) {
      return const SizedBox.shrink();
    }
    final challengeIndex = iterationDocs.indexWhere((d) => d.reference.id == iterationId);
    final challengeLabel = challengeIndex != -1 ? 'challenge ${challengeIndex + 1}' : '';
    ...
    for (final session in _sessions) {
      for (final shot in session.shots ?? const <Shots>[]) {
  ...
  List<ShootingSession> _loadSessionsWithShotsForType(List<ShootingSession> sessions, String shotType) {
    return sessions.where((s) => (s.shots ?? const <Shots>[]).any((shot) => shot.type == shotType && shot.targetsHit != null && shot.count != null && shot.count! > 0)).toList();
  }
}
