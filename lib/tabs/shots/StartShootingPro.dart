...
class StartShootingPro extends StatefulWidget {
  const StartShootingPro({super.key, required this.sessionPanelController, this.shots});

  final PanelController sessionPanelController;
  final List<Shots>? shots;

  @override
  State<StartShootingPro> createState() => _StartShootingProState();
}

class _StartShootingProState extends State<StartShootingPro> with SingleTickerProviderStateMixin {
  ...
  int calibrationShotsNeeded = 12; // e.g., 12 per type
  Map<String, List<dynamic>> shotAudioSamples = {
    'wrist': [],
    'snap': [],
    'slap': [],
    'backhand': [],
  };
  bool autoShotCountingEnabled = false;
  int autoShotCount = 0;
  String autoShotType = 'wrist';
  DateTime? lastShotDetected;
  ...
  void _animateNotificationTrayOut() {
    setState(() {
      _notificationTrayAnimatingOut = true;
    });
    _notifAnimController.forward(from: 0).then((_) {
      setState(() {
  ...
  void _showNotificationTrayAgain() {
    setState(() {
      _showNotificationTray = true;
      _notifAnimController.reverse(from: 1);
    });
  }

  // --- Pro Feature: Calibration UI ---
  Widget _buildProCalibrationWidget(BuildContext context) {
    ...
    if (!isCalibrating) {
      return Card(
        ...
        child: ListTile(
          leading: const Icon(Icons.mic, color: Colors.amber),
          title: Text(
            "Try Pro: Auto Shot Counting",
            style: TextStyle(
              color: Colors.deepOrange.shade800, // More distinct orange text
              fontFamily: 'NovecentoSans',
              fontWeight: FontWeight.bold,
            ),
          ),
          ...
          trailing: ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.deepOrange.shade400, // Strong orange button
      ...
      return Card(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                "Calibration: ${calibratingShotType![0].toUpperCase() + calibratingShotType!.substring(1)} Shot",
                style: TextStyle(
                  fontFamily: 'NovecentoSans',
                  fontWeight: FontWeight.bold,
                  fontSize: 18,
              ...
              Text(
                "Take a $calibratingShotType shot and let the app listen. (${calibrationStep + 1}/$calibrationShotsNeeded)",
                style: const TextStyle(fontSize: 15),
              ),
              const SizedBox(height: 12),
              ElevatedButton.icon(
                icon: const Icon(Icons.mic),
                label: const Text("Record Shot Sound"),
                onPressed: () async {
                  // TODO: Record short audio sample and store in shotAudioSamples[calibratingShotType]
                  // For now, just simulate
                  setState(() {
                    shotAudioSamples[calibratingShotType!]!.add('audio_sample_${calibrationStep + 1}');
                    calibrationStep++;
                  });
                  if (calibrationStep + 1 > calibrationShotsNeeded) {
                    // Move to next type or finish
                    final nextType = _nextShotType(calibratingShotType!);
                    if (nextType != null) {
                      setState(() {
                        calibratingShotType = nextType;
                        calibrationStep = 0;
                      });
                    } else {
                      // All done
                      _onCalibrationComplete();
                      // TODO: Train model with shotAudioSamples
                    }
                  }
                },
              ),
              const SizedBox(height: 8),
              if (calibratingShotType != null)
                TextButton(
                  onPressed: () {
                    setState(() {
                      isCalibrating = false;
  ...
  String? _nextShotType(String current) {
    const order = ['wrist', 'snap', 'slap', 'backhand'];
  ...
  Widget _buildNotificationOverlay(BuildContext context) {
    ...
    return AnimatedPositioned(
      ...
      child: AnimatedOpacity(
        ...
        child: Material(
          color: Colors.transparent,
          child: _buildNotificationTray(context),
        ),
      ),
    );
  }
  ...
  // Shot type selector widget (copied from StartShooting.dart)
