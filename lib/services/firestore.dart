...
Future<bool> saveShootingSession(
  ...
  ShootingSession shootingSession = ShootingSession(
    ...
    isChallengerRoad: isChallengerRoad ? true : null,
  );
  ...
  return await ref.collection('sessions').add(shootingSession.toMap()).then((s) async {
    ...
    await ref.get().then((i) {
      ...
      iteration = Iteration(
        ...
        (iteration.total ?? 0) + (shootingSession.total ?? 0),
        (iteration.totalWrist ?? 0) + (shootingSession.totalWrist ?? 0),
        (iteration.totalSnap ?? 0) + (shootingSession.totalSnap ?? 0),
        (iteration.totalSlap ?? 0) + (shootingSession.totalSlap ?? 0),
        (iteration.totalBackhand ?? 0) + (shootingSession.totalBackhand ?? 0),
        iteration.complete,
        iteration.udpatedAt,
      );
